...
def copyRange(copyDict, sheet):
    ...
    for i in range(12,14,1):
        ...
        for j in copyDict:
            if j != "Date":
                rowSelected[j] =sheet.cell(row = i , column= copyDict[j]).value
            else:
                d=sheet.cell(row = i , column= copyDict[j]).value
...
def pasteRange(copyDict, pasteDict, sheetReceiving, copiedData):
    ...
    for i in range(12,14,1):
        for j in copyDict:
            sheetReceiving.cell(row = i, column = pasteDict[j] ).value = copiedData[countRow][j]
...
# newCopyDict= dictValidator(copyDict)
# newPasteDict= dictValidator(pasteDict)
# print(newPasteDict)
# print(newCopyDict)
# createData()
...
# print(copyRange(copyDict, copySheet))